...
function defaultUserAgent() {
  return process.env.REDDIT_USER_AGENT || "web:daily-tairazuma:1.0 (archive reader)";
}
...
export function buildSearchUrl(after = null) {
...
async function fetchRedditJsonPosts(fetchImpl, userAgent = defaultUserAgent()) {
...
export async function fetchRedditRss(fetchImpl = fetch, { userAgent = defaultUserAgent() } = {}) {
  const params = new URLSearchParams(SEARCH_PARAMS);
